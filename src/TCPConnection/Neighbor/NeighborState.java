...
public class NeighborState {
    ...
    public synchronized void recievedBitfield(byte [] bitfield){
    ...
    public int getRandomPiece(FileParser file){
       return file.getRandomPiece(bitfield);
    }
    ...
    public boolean checkIfInterested(FileParser file){
        ...
        for(int i=0;i<file.getNumberOfPieces();++i){
            if(bitfield.get(i)==true && numberOfPiecesInClientPossesion.get(i)==false && file.getPiecesAlreadyRequested().get(i)==false){
                return true;
            }
        }
        return false;
